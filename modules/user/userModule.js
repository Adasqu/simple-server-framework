...
import usersModel from "../../models/user/userModel.js";
import ordersModel from "../../models/order/orderModel.js";
import responsError from "../errors.js";
import mail from "../mailer.js";
var userModel;
var fastify;

export async function userModule(app) {
  fastify = app;
  userModel = usersModel(app);
  userModel.hasMany(ordersModel(app))
...
export async function forgetPassword(req, res) {
  try {
    ...
    if (Object.keys(user).length === 0) throw "Not found";
    res.send({ status: "ok", data: "User password recovery token sent" });
  } catch (error) {
    responsError(error, res);
  }
}
